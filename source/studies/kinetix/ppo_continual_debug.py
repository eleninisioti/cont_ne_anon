...
def make_train(config, env_params, static_env_params):
    ...
    def train(rng):
        ...
        train_state, env_state, last_obs_one, rng = runner_state
        
        
        # second round
        config["train_level"] = "m"
        env = make_env(make_reset_fn_from_config(config, env_params, static_env_params), static_env_params)
        ...
        train_state = TrainState.create(
            ...
            params=train_state.params,  # Keep the learned parameters
            tx=tx,  # Fresh optimizer state
        )
        runner_state = (train_state, env_state_two, last_obs_two, rng)
        
        runner_state, metric = jax.lax.scan(
            _update_step_two, runner_state, None, config["NUM_UPDATES"]/2
        )
        all_metrics.append(metric)
        train_state, env_state_two, last_obs_two, rng = runner_state
...
def main():
    # Hardcoded debug arguments
    config = OmegaConf.create({
        "train_levels": "s",
        "train_levels_list": ["s/h2_one_wheel_car.json"],
        "continual": True,
        "load_from_checkpoint": "<wandb-entity>/Kinetix-experiments/Kinetix-SymbolicEntity-MultiDiscrete-PPO-1B-2048_h1_thrust_over_ball_continual_True-529392377-0B-checkpoint:v0",
        "load_only_params": True
    })
    
    # Load base config and merge with hardcoded values
    base_config = OmegaConf.load("configs/ppo.yaml")
