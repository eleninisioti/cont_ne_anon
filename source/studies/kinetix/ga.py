...
"""GA training on Kinetix environments.
...
def build_config_for_env(env_name):
    ...
    """
    # Locate the yaml shipped in ga_example (fallback: third_party/kinetix root)
    yaml_candidates = [
...
def train_ga(
    ...
    *,
    ...
    2. Create SimpleGA strategy (source/algorithms/ne/ga.py)
    ...
    print(f"  num_elites (archive size): {strategy.num_elites}")
    ...
    for gen in range(generations):
        ...
        # tell – SimpleGA *minimises*, so negate the *evolve* fitness
        es_state = strategy.tell(flat_pop, -evolve_fit, es_state, es_params)
