...
from source.studies.brax.my_brax import networks  # Use our local networks module
...
def make_ppo_networks(
    ...
    redo_tau: float = networks.REDO_DEFAULT_TAU,
) -> PPONetworks:
  """Make PPO networks with preprocessor.

  policy_min_std floors the action standard deviation: brax parameterises it as
  softplus(net output) + min_std, so a min_std of 0.3 makes sigma >= 0.3 no
  matter what the policy head learns. brax's default of 0.001 places no floor at
  all, and with entropy_cost 0 nothing else opposes the collapse of sigma --
  which is exactly what stops this PPO learning walker (see
  docs/cchain_walker_reproduction.md). The knob exists to separate "the entropy
  BONUS was needed" from "some floor on exploration was needed", which an
  entropy_cost sweep alone cannot distinguish.
  """
