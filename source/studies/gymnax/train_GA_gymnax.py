...
def parse_args():
    ...
    parser.add_argument('--variation', type=str, default=GAUSSIAN,
                        ...
                        help="how offspring are bred: 'gaussian' is this GA as "
                             "published, 'isoline' swaps in DNS's Iso+LineDD "
                             'and changes nothing else. The stationary half of '
                             'the operator ablation; see '
...
def main():
    ...
    for gen in range(num_generations):
        ...
        population, ga_state, parents = jit_ask(ask_key, ga_state, ga_params)
        ...
        # coordinate-wise mean of the ELITE ARCHIVE. Read it as "has the
        # archive collapsed onto one solution", never as performance --
        ...
        _elite_flat = population_host[int(np.argmax(mean_fitness_host))]
        plast_metrics = plast.update(
            jax.random.fold_in(plasticity_key, gen), population_host, _elite_flat,
        ...
        # and the one that catches a population whose norms grow without bound.
        # Every generation, unlike the brax ant trainers which gate it on
        # --diversity_interval: this is a numpy reduction over a 512 x ~5k
        # array, which is nothing next to the rollouts, and it needs no probe
        # batch so it is defined even with --track_diversity 0.
