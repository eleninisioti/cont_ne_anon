...
def _check_model_matches_reference(model):
    """Fail loudly if a run's network differs from the one every method uses.

    The kinetix trainers reach their config two ways: PPO / PBT compose hydra's
    `configs/`, while GA / ES / DNS load the flat `kinetix_config_pixels.yaml`.
    Those are two copies of the same architecture, and a comparison across the
    methods is only about the method while they agree. They do agree today; this
    is what stops one of them being edited alone.

    Checked here because `normalise_config` is the one call every kinetix
    trainer makes. Changing the architecture means changing both files -- and
    re-running every kinetix arm, since it is a method change.
    """
    reference = os.path.join(os.path.dirname(__file__), "..", "..", "configs", "model", "model-base.yaml")
    reference = os.path.normpath(reference)
    if not os.path.exists(reference):  # installed without the configs tree
        return
    with open(reference) as f:
        want = yaml.safe_load(f)
    mismatched = {
        k: (model.get(k), v) for k, v in want.items()
        if k in model and model[k] != v
    }
    if mismatched:
        detail = ", ".join(f"{k}: config has {got!r}, {reference} has {exp!r}"
                           for k, (got, exp) in sorted(mismatched.items()))
        raise ValueError(
            "kinetix network architecture disagrees with the shared reference, so "
            "this run would not be comparable against the other methods on the same "
            f"task: {detail}. Change both files, or neither."
        )


def normalise_config(config, name, editor_config=False):
    old_config = copy.deepcopy(config)
    _check_model_matches_reference(config.get("model") or {})
