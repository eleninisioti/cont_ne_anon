...
def make_network_from_config(env, env_params, config, network_kws={}, actor_only=False):
    ...
    # Enable counting if either ReDo is enabled OR monitoring is explicitly enabled
    monitor_dormant = config.get("monitor_dormant", False)
    use_redo = config.get("use_redo", False)
    network_kws["count_dormant"] = use_redo or monitor_dormant
    network_kws["redo_tau"] = config.get("redo_tau", REDO_DEFAULT_TAU)
    # "variability" (the default) scores std(activation); "magnitude" scores
    # mean|activation|. These networks are tanh, where a saturated unit has the
    # largest possible magnitude and so is invisible to the magnitude test --
    # see kinetix/models/actor_critic.py for the full reasoning.
    network_kws["dormant_criterion"] = config.get("dormant_criterion", "variability")

    obs_type = config["observation_type"]
    assert obs_type in ObservationType and type(obs_type) != str
    if obs_type == ObservationType.PIXELS:
        cls_to_use = ActorOnlyPixelsRNN if actor_only else ActorCriticPixelsRNN
    elif obs_type == ObservationType.SYMBOLIC_FLAT:
        cls_to_use = ActorCriticSymbolicRNN  # No actor-only version yet
