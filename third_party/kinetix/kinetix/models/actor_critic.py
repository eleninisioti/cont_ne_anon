...
# The dormancy score is defined once, in source/algorithms/rl/redo.py, so gymnax, mujoco,
# brax and kinetix all count a dormant neuron the same way. These names are
# re-exported because the modules below and kinetix/models/redo.py refer to them
# by their kinetix spelling. `variability` is the default here because these
# networks are tanh (configs/model/model-base.yaml); see core/redo.py for why
# the criterion follows the activation.
from source.algorithms.rl.redo import (  # noqa: E402
    CRITERION_MAGNITUDE as DORMANT_CRITERION_MAGNITUDE,
    CRITERION_VARIABILITY as DORMANT_CRITERION_VARIABILITY,
    DEFAULT_TAU as REDO_DEFAULT_TAU,
    neuron_dormancy_score as _core_neuron_dormancy_score,
)


def neuron_dormancy_score(act, criterion=DORMANT_CRITERION_VARIABILITY):
    return _core_neuron_dormancy_score(act, criterion)
...
class GeneralActorCriticRNN(nn.Module):
    ...
    redo_tau: float = REDO_DEFAULT_TAU  # Dormancy threshold, as a fraction of the layer mean
    dormant_criterion: str = DORMANT_CRITERION_VARIABILITY
    ...
    def __call__(self, hidden, obs, embedding, dones, activation):
        ...
        for _ in range(self.fc_layer_depth):
            ...
            if self.count_dormant:
                per_neuron_act = neuron_dormancy_score(actor_mean, self.dormant_criterion)
                ...
                # Publish the per-neuron score so ReDo can rebuild the mask and
                # actually recycle the neurons. `sow` is used rather than an
                # extra return value because ppo.py, ga.py, dns.py, plr.py and
                # sfl.py all unpack this module's 4-tuple, and one of them
                # unpacks the dormant-counts element into exactly two names.
                self.sow("intermediates", "policy_neuron_score", per_neuron_act)
            ...
            if self.count_dormant:
                per_neuron_act = neuron_dormancy_score(critic, self.dormant_criterion)
                dormant_mask = (per_neuron_act <= self.redo_tau)
                value_n_dormant += jnp.sum(dormant_mask.astype(jnp.int32))
                value_n_neurons += critic.shape[-1]
                self.sow("intermediates", "value_neuron_score", per_neuron_act)
...
class ActorCriticPixelsRNN(nn.Module):
    ...
    def __call__(self, hidden, x, **kwargs):
        ...
        return GeneralActorCriticRNN(
            ...
            # These three were missing here, though ActorCriticSymbolicRNN and
            # ActorCriticPermutationInvariantSymbolicRNN below both forward
            # them. The inner module therefore fell back to count_dormant=False
            # on every PIXELS run -- which is every Kinetix RL run -- so the
            # "dormant fraction" reported was the hardcoded 0.0 from the
            # else-branch rather than a measurement, and ReDo had no neuron
            # scores to act on. Any dormancy number from a pixels run before
            # this fix is that constant, not data.
            count_dormant=self.count_dormant,
            redo_tau=self.redo_tau,
            dormant_criterion=self.dormant_criterion,
...
class GeneralActorOnlyRNN(nn.Module):
    ...
    recurrent: bool = False
    # Dormancy counting, mirroring GeneralActorCriticRNN above. The actor-only
    # network is what the NE trainers (ga/dns/es_continual.py) evolve, and
    # without these the NE side of kinetix could report no dormancy at all while
    # the RL side reported it every update -- so an NE-vs-RL plasticity
    # comparison was impossible on this suite alone.
    #
    # The score is published with `sow` rather than returned, deliberately:
    # every caller unpacks this module's 2-tuple `(hidden, pi)`, and widening
    # that would break all of them. `sow` leaves the signature untouched and the
    # caller opts in with `mutable=["intermediates"]`.
    count_dormant: bool = False
    redo_tau: float = REDO_DEFAULT_TAU
    dormant_criterion: str = DORMANT_CRITERION_VARIABILITY
    ...
    def __call__(self, hidden, og_obs, embedding, dones, activation):
        ...
        for i in range(self.fc_layer_depth):
            ...
            # Same criterion, tau and scoring function as the actor-critic path,
            # so an NE dormancy column and an RL dormancy column are the same
            # measurement. `variability` (std of activation) rather than
            # magnitude: these nets are tanh, where a saturated unit has the
            # LARGEST magnitude and is invisible to a magnitude test.
            if self.count_dormant:
                self.sow(
                    "intermediates", "policy_neuron_score",
                    neuron_dormancy_score(actor_mean, self.dormant_criterion),
                )
...
class ActorOnlyPixelsRNN(nn.Module):
    ...
    generator_embedding_number_of_timesteps: int = 10
    recurrent: bool = False
    count_dormant: bool = False  # Forwarded to GeneralActorOnlyRNN, which sows per-neuron scores
    redo_tau: float = REDO_DEFAULT_TAU
    dormant_criterion: str = DORMANT_CRITERION_VARIABILITY
